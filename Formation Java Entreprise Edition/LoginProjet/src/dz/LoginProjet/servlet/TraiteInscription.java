...
public class TraiteInscription extends HttpServlet {
	...
	protected void doGet(HttpServletRequest req, HttpServletResponse resp)
		...
		if (prenom.trim().isEmpty() || tel.trim().isEmpty()
			...
			message = "Votre demande a été bien traitée";
			req.setAttribute("m", message);
			...
			if (s.getDomaine() != "Informatique") {
				message2 = "Vous n'êtes pas dans le domaine informatique, Veuilllez passer à l'école pour passer un test sur les notions algorithmique";
				req.setAttribute("msg", message);
				this.getServletContext().getRequestDispatcher(
						"/WEB-INF/Accueil.jsp");
			}
			if (s.getQuestion() != "Oui") {
				message2 = "Pour s'inscrire à la formation PHP vous devez d'abord faire la formation HTML/CSS";
				req.setAttribute("msg", message);
				this.getServletContext().getRequestDispatcher(
						"/WEB-INF/Accueil.jsp");
			}
